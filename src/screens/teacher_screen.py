...
try:
    ...
    def voice_attendance_dialog(selected_subject_id):
        st.info("Voice Attendance is available when librosa audio packages are loaded.")
    ...
    if subjects:
        for sub in subjects:
            ...
            def share_btn(name=sub['name'], code=sub['subject_code']):
                if st.button(f"📤 Share Code: {name}", key=f"share_{code}"):
...
def teacher_screen_login():
    ...
    teacher_username = st.text_input("Enter username", placeholder="Sundar Pichai")
    teacher_pass = st.text_input("Enter password", type="password", placeholder="Enter your password")
    st.divider()
    
    btn1, btn2 = st.columns(2)
    with btn1:
        if st.button("🔑 Login", shortcut="control+enter", use_container_width=True):
    ...
    with btn2:
        if st.button("📝 Register Instead", type='primary', use_container_width=True): 
...
def teacher_screen_register():
    ...
    teacher_username = st.text_input("Enter username", placeholder="Sundar Pichai")
    teacher_name = st.text_input("Enter name", placeholder="Sundar Pichai")
    teacher_pass = st.text_input("Enter password", type="password", placeholder="Enter your password")
    ...
    btn1, btn2 = st.columns(2)
    with btn1:
        if st.button("✨ Register now", shortcut="control+enter", use_container_width=True):
    ...
    with btn2:
        if st.button("🔑 Login Instead", type='primary', use_container_width=True):
            st.session_state.teacher_login_type = "login"
